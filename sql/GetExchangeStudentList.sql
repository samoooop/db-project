select s.sid ,s.first_name, s.last_name, Grade.GPAX as gpax , count(g.rewardid) as rewardAmount ,s.entry_year as enYear,t.
from  student s left join got_reward g on s.sid = g.studentid left join teacher t on t.tid = s.tid,take_leave tl,
