...
from major m  inner join major_req_course mrc on m.mid =mrc.req_mid
        ...
        left outer join (	select  sum(scc.grade * c.credit)/sum(c.credit) AS avgGrade, c.cid as cid
						...
						where 	substring(scc.semester_id,1,4) = year(current_date()) + 543
								and scc.status = 'P'
						group by scc.course_id ) as avgGrade on avgGrade.cid = c.cid and avgGrade.cid = mrc.req_cid
        ...
        left outer join (	select scc.status ,scc.course_id as cid 
							...
							where scc.status='W' and substring(scc.semester_id,1,4) = year(current_date()) + 543 ) as ccw on c.cid = ccw.cid and ccw.cid = mrc.req_cid and ccw.cid = avgGrade.cid
		left outer join ( select scc.status ,scc.course_id as cid 
							from sem_consist_course scc 
							where scc.status='P' and scc.grade = 0 and substring(scc.semester_id,1,4) = year(current_date())+ 543 ) as ccf on c.cid = ccf.cid and ccf.cid = mrc.req_cid and ccf.cid = avgGrade.cid and ccf.cid = ccw.cid    
